...
. Run the `seedu.equipmentmanager.MainApp` and try a few commands
...
Step 2. The user executes `filter north ongoing`
...
// tag::sort[]
=== Sort feature
The sort feature allow users to sort the current shown list by name.
...
Step 2. The user executes `delete 5` command to delete the 5th equipment in the address book. The `delete` command calls `Model#commitAddressBook()`, causing the modified state of the address book after the `delete 5` command executes to be saved in the `addressBookStateList`, and the `currentStatePointer` is shifted to the newly inserted address book state.

image::UndoRedoNewCommand1StateListDiagram.png[width="800"]

Step 3. The user executes `add n/David ...` to add a new equipment. The `add` command also calls `Model#commitAddressBook()`, causing another modified address book state to be saved into the `addressBookStateList`.
...
.. _Unit tests_ that test the individual components. These are in `seedu.equipmentmanager.ui` package.
.  *Non-GUI Tests* - These are tests not involving the GUI. They include,
..  _Unit tests_ targeting the lowest level methods/classes. +
e.g. `seedu.equipmentmanager.commons.StringUtilTest`
..  _Integration tests_ that are checking the integration of multiple code units (those code units are assumed to be working). +
e.g. `seedu.equipmentmanager.storage.StorageManagerTest`
..  Hybrids of unit and integration tests. These test are checking multiple code units as well as how the are connected together. +
e.g. `seedu.equipmentmanager.logic.LogicManagerTest`
